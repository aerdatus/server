var Node = function(node, icon, stationMarker) {
  ...
  var nodePoint = new google.maps.LatLng(node.location.latitude, node.location.longitude);
...
Node.prototype.draw = function() {
  ...
  google.maps.event.addListener(this.nodeMarker, 'click', function() {
    ...
    self.infowindow = new google.maps.InfoWindow({
      content: '<b>ID:</b> ' + this.node.id + '</br><b>Firstseen:</b> ' + this.node.firstseen + '</br><b>Lastseen:</b> ' + this.node.lastseen + '</br><b>Power:</b> ' + this.node.power + '</br><b>Probes:</b> ' + this.node.probes
    });
...
Node.prototype.drawLocations = function() {
  if (this.node.location && this.node.location.latitude && this.node.location.longitude) {
    var circleOptions1 = {
      strokeColor: '#3366FF',
      strokeOpacity: 0,
      strokeWeight: 1,
      ...
      fillOpacity: 0.5,
      map: map,
      center: new google.maps.LatLng(this.node.location.latitude, this.node.location.longitude),
  ...
  for (var i = 0; i < this.node.shown.length; i++) {
    ...
    if (show.location.latitude && show.location.longitude) {
      var circleOptions2 = {
        ...
        fillOpacity: 0.5,
        map: map,
        center: new google.maps.LatLng(show.location.latitude, show.location.longitude),
